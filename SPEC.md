# Claude Code Night Shift — מסמך איפיון טכני
...
| `webview/night-shift.js` | Webview / browser context (תוך ה-DOM של פאנל Claude Code) | כל הלוגיקה התלויה-DOM: כפתור ON/OFF, מנוע פינג, זיהוי מצב (מבוסס session-state), זיהוי rate limit, תזמון חידוש, **ה-kill-switch הקשיח**. |
...
* **העברת קונפיגורציה:** ה-Host כותב `window.__NIGHTSHIFT_CONFIG__ = JSON.stringify(...)` לפני הסקריפט המוזרק (כמו `__RTL_CONFIG__`). seed ראשוני בלבד; חובה JSON.stringify כדי למנוע injection דרך `pingText`.
...
├─ webview/
│  └─ night-shift.js            # הסקריפט המוזרק (browser context, IIFE)
├─ media/
└─ test/
```
...
* שמירת גיבוי: `index.js.nightshift-backup` (לא `index.js.backup` — תפוס ע"י RTL; ראו 7).
...
```JavaScript
// >>> Claude Code Night Shift (injected) v<X.Y.Z> >>>
window.__NIGHTSHIFT_CONFIG__ = JSON.parse("...");  // seed, JSON.stringify מצד ה-Host
(function(){ /* תוכן webview/night-shift.js */ })();
// <<< Claude Code Night Shift (injected) <<<
```
...
טבלת קונפיג (`window.__NIGHTSHIFT_CONFIG__`, וגם `contributes.configuration` ב-package.json):
...
* `nightshift-enabled` — `"true"`/`"false"`.
* `nightshift-sleep-until` — timestamp (ms) של השכמה אחרי rate limit, או ריק.
* `nightshift-session-start` — תחילת המשמרת (לאכיפת maxRuntime).
* `nightshift-ping-count` — מונה פינגים במשמרת.
* `nightshift-owner-id` — מזהה ה-instance שמנהל את המשמרת (לריבוי-פאנלים; ראו 4.7).
...
* **Sentinel (מנגנון מומלץ):** הפינג כולל הוראה ש-Claude יכתוב מחרוזת סימן (למשל `NIGHTSHIFT_DONE`) כשהמשימה הושלמה; ה-webview מזהה אותה ועוצר. זהו אות ה-done **האמין ביותר** — מקודם מ"אופציונלי עתידי" ל**ברירת מחדל מומלצת** (כפוף להסכמת המשתמש על ניסוח הפינג).
...
> **תיקון קריטי מהביקורת:** בדיקת "ה-marker קיים?" אינה מספיקה. הבדיקה חייבת לאמת **invariant**: קיים *בדיוק זוג markers אחד* תקין (פתיחה+סגירה), בגרסה הנוכחית, ושום שריד Night Shift אחר מחוץ לו.

* אם ה-invariant מתקיים בגרסה הנוכחית → לא לעשות כלום.
* אחרת: לחתוך **את כל** זוגות ה-markers של Night Shift (כולל שרידים/כפילויות מ-restore של RTL), לוודא גיבוי (6.4), ולהוסיף בלוק עדכני יחיד.
* **אסור** לחתוך עד-סוף-הקובץ — תמיד מ-marker-פתיחה עד marker-סגירה בלבד, לכל המופעים.

### 6.4 גיבוי וכתיבה אטומית

* שם גיבוי: `index.js.nightshift-backup`.
...
* פקודה "Remove Night Shift Injection": לחתוך רק את זוגות ה-markers שלנו (לא לשחזר מ-backup עיוור — שלא למחוק RTL שהוזרק אחרינו).
* למחוק `index.js.nightshift-backup`.
...
* RTL מזריק ל**שני** קבצים: `webview/index.js` (marker `RTL for VS Code Agents`, גיבוי `index.js.backup`) **וגם** `extension.js` (marker `RTL-Plan-Injection`, גיבוי `extension.js.rtl-backup`). **Night Shift נוגע רק ב-`webview/index.js`** ולא ב-`extension.js` — שם אין בינינו התנגשות.
...
1. **שם backup נפרד** (`index.js.nightshift-backup`); לעולם לא לגעת ב-`index.js.backup` של RTL.
...
4. **תיקון סדר:** מכיוון ש-RTL חותך עד-סוף-קובץ ב-restore, בלוק Night Shift שנמצא **אחרי** ה-marker של RTL **יימחק** בשחזור RTL; בלוק **לפני** ה-marker של RTL ישרוד. כלומר דווקא הסדר "אחרי RTL" הוא ה**מסוכן**. אין סדר "בטוח" מובטח → ההגנה האמיתית = idempotency מבוסס-מיקום (6.3) + reinject תקופתי.

### 7.2 נתיב שחיתות אמיתי (לא רק "סיכון שיורי")

אם RTL גיבה *אחרי* שהזרקנו ואז משחזר: הקובץ המשוחזר עלול להכיל בלוק Night Shift "מאובן" ש-RTL לא יסיר (כי הוא חותך ב-marker של RTL, שאחרי שלנו). **idempotency נאיבי ("ה-marker קיים → לדלג") יותיר את השריד.** לכן 6.3 מחייב invariant מבוסס-מיקום שמזהה ומנקה שרידים/כפילויות. בנוסף יש **write race** (שני התוספים כותבים בלי נעילה) → כתיבה אטומית + verify-after-write (6.4).
...
* **Host (Node):** יוניט ל-`injector` (inject/strip/restore, חיתוך דו-צדדי, **invariant מבוסס-מיקום**, ניקוי כפילויות, כתיבה אטומית) ול-`coexistence` על fixtures — כולל fixture עם RTL לפני/אחרי, ו-fixture עם שריד Night Shift "מאובן". ובדיקת תרחיש: RTL `restoreAllBackups` → reinject שלנו מנרמל.
